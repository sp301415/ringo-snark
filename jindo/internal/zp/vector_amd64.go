...
//go:noescape
func addVec(res, a, b *Uint, n uint64)
...
//go:noescape
func scalarMulVec(res, a, b *Uint, n uint64, qInvNeg uint64, patterns *uint64)

// Sum computes the sum of all elements in the vector.
func (vector *Vector) Sum() (res Uint) {
	n := uint64(len(*vector))
	if n == 0 {
		return
	}
...
//go:noescape
func sumVec(res *Uint, a *Uint, n uint64)

// InnerProduct computes the inner product of two vectors.
// It panics if the vectors don't have the same length.
func (vector *Vector) InnerProduct(other Vector) (res Uint) {
	n := uint64(len(*vector))
	if n == 0 {
		return
	}
...
//go:noescape
func innerProdVec(res *uint64, a, b *Uint, n uint64)
...
//go:noescape
func mulVec(res, a, b *Uint, n uint64, qInvNeg uint64, patterns *uint64)
