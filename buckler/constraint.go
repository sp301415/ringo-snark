...
type AritmeticConstraint[E num.Uint[E]] struct {
	rank                int
	coeffs              []E
...
func (c *AritmeticConstraint[E]) AddTerm(coeff E, coeffPublicWitness PublicWitness[E], witness ...Witness[E]) {
	c.coeffs = append(c.coeffs, coeff)

	digits := make([]uint64, coeff.Limb())
	if coeffPublicWitness != nil {
		coeffPublicWitness[0].Slice(digits)
		c.coeffsPublicWitness = append(c.coeffsPublicWitness, digits[1])
	} else {
