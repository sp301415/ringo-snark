...
// Uint represents a field element stored on 7 words (uint64)
//
// Uint are assumed to be in Montgomery form in all methods.
//
// Modulus q =
//
//	q[base10] = 1419612551513299965208849894211363347143714633506572244659872229707204744949688948107755123067539182006159147532689398427519429902337
//	q[base16] = 0x8000217e58e6d46dbee62ad4891f120a5c6f419499b3ef452ff53b892cf18d035c0112137e12bda25a7236cea5c7c596baed8100000001
//
// # Warning
//
...
const (
	Limbs = 7   // number of 64 bits words needed to represent a Uint
	Bits  = 440 // number of bits needed to represent a Uint
	Bytes = 56  // number of bytes needed to represent a Uint
)

// Field modulus q
const (
	q0 = 10861254589663936513
	q1 = 11698788561769056197
	q2 = 242069657119232701
	q3 = 4985472948575728013
	q4 = 746593965693318127
	q5 = 7908011067337940754
	q6 = 36028940872640212
)
...
const qInvNeg = 10861254589663936511

func init() {
	_modulus.SetString("8000217e58e6d46dbee62ad4891f120a5c6f419499b3ef452ff53b892cf18d035c0112137e12bda25a7236cea5c7c596baed8100000001", 16)
}
...
func (z *Uint) SetInterface(i1 interface{}) (*Uint, error) {
	if i1 == nil {
		return nil, errors.New("can't set zp440.Uint with <nil>")
	}
	...
	default:
		return nil, errors.New("can't set zp440.Uint from type " + reflect.TypeOf(i1).String())
	}
}
...
func (z *Uint) SetOne() *Uint {
	z[0] = 2368870868303478273
	z[1] = 17110868891616558232
	z[2] = 5429613728038950776
	z[3] = 16520749523430659974
	z[4] = 5872109078615020900
	z[5] = 17290040806413630205
	z[6] = 35955287790403064
	return z
}
...
func (z *Uint) IsOne() bool {
	return ((z[6] ^ 35955287790403064) | (z[5] ^ 17290040806413630205) | (z[4] ^ 5872109078615020900) | (z[3] ^ 16520749523430659974) | (z[2] ^ 5429613728038950776) | (z[1] ^ 17110868891616558232) | (z[0] ^ 2368870868303478273)) == 0
}
...
func (z *Uint) LexicographicallyLargest() bool {
	...
	var b uint64
	_, b = bits.Sub64(_z[0], 14653999331686744065, 0)
	_, b = bits.Sub64(_z[1], 15072766317739303906, b)
	_, b = bits.Sub64(_z[2], 9344406865414392158, b)
	_, b = bits.Sub64(_z[3], 11716108511142639814, b)
	_, b = bits.Sub64(_z[4], 373296982846659063, b)
	_, b = bits.Sub64(_z[5], 3954005533668970377, b)
	_, b = bits.Sub64(_z[6], 18014470436320106, b)
...
func (z *Uint) SetBytesCanonical(e []byte) error {
	if len(e) != Bytes {
		return errors.New("invalid zp440.Uint encoding")
	}
	v, err := BigEndian.Element((*[Bytes]byte)(e))
	if err != nil {
		return err
...
var errInvalidEncoding = errors.New("invalid zp440.Uint encoding")
...
func init() {
	_bLegendreExponentUint, _ = new(big.Int).SetString("400010bf2c736a36df73156a448f89052e37a0ca4cd9f7a297fa9dc49678c681ae008909bf095ed12d391b6752e3e2cb5d76c080000000", 16)
	const sqrtExponentUint = "400010bf2c736a36df73156a448f89052e37a0ca4cd9f7a297fa9dc49678c681ae008909bf095ed12d391b6752e3e2cb5d76c0"
...
func (z *Uint) Legendre() int {
	...
	for !a.IsZero() {
		...
		a[6] = (a[6] >> nbIterations) | (aHi << (2*k - nbIterations))
		...
		b[6] = (b[6] >> nbIterations) | (bHi << (2*k - nbIterations))
	}

	if b[0] == 1 && (b[1]|b[2]|b[3]|b[4]|b[5]|b[6]) == 0 {
		return l // (0|1) = 1
	} else {
...
func (z *Uint) Sqrt(x *Uint) *Uint {
	...
	// g = nonResidue ^ s
	var g = Uint{
		10675110718625115766,
		6979866634609968241,
		7864021933611599009,
		17342526839974424205,
		9655998928190658930,
		7554881204491203732,
		1489483447421385,
	}
...
const (
	inversionCorrectionFactorWord0 = 9783283685750003722
	inversionCorrectionFactorWord1 = 3102502472558114173
	inversionCorrectionFactorWord2 = 14438795493831480127
	inversionCorrectionFactorWord3 = 14983722312752916138
	inversionCorrectionFactorWord4 = 601848296183389140
	inversionCorrectionFactorWord5 = 71256047383913589
	inversionCorrectionFactorWord6 = 23085712747985865
	invIterationsN                 = 30
)
