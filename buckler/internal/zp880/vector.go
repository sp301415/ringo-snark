...
// Vector represents a slice of Uint.
//
...
// WriteTo implements io.WriterTo and writes a vector of big endian encoded Uint.
...
func (vector *Vector) AsyncReadFrom(r io.Reader) (int64, error, chan error) { // nolint ST1008
	...
	*vector = (*vector)[:0]
	...
	for i := uint64(0); i < headerSliceLen; i += maxAllocateSliceLength {
		if len(*vector) <= int(i) {
			(*vector) = append(*vector, make([]Uint, int(min(headerSliceLen-i, maxAllocateSliceLength)))...)
		}
...
func scalarMulVecGeneric(res, a Vector, b *Uint) {
	if len(a) != len(res) {
...
func sumVecGeneric(res *Uint, a Vector) {
	for i := 0; i < len(a); i++ {
		res.Add(res, &a[i])
	}
}

func innerProductVecGeneric(res *Uint, a, b Vector) {
	if len(a) != len(b) {
